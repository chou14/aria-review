...
function renderGapPanel(ui: ReactElement) {
  return render(<MemoryRouter>{ui}</MemoryRouter>);
}

describe("GapPanel", () => {
  it("按 theme 分组渲染 + 计数 + 诚信说明（置信非裁决）", () => {
    renderGapPanel(<GapPanel projectId={5} gaps={ALL_GAPS} />);
    // 两个主题簇都出现
  ...
  it("展开卡片 → 见支撑引文 + Paper 跳转 href + 源坐标锚点 + 反证", () => {
    render(
      <MemoryRouter initialEntries={["/projects/5/research"]}>
        <GapPanel projectId={5} gaps={ALL_GAPS} />
        <LocationProbe />
      </MemoryRouter>,
    );
    // 折叠态：支撑引文不在 DOM
    ...
    expect(screen.getByTestId("location").textContent).toBe("/projects/5/library/7");
  ...
  it("空 / 加载 / 错误态", () => {
    const { rerender } = renderGapPanel(<GapPanel projectId={5} gaps={[]} />);
    expect(screen.getByText("尚未发现研究空白")).toBeInTheDocument();

    rerender(<MemoryRouter><GapPanel projectId={5} gaps={[]} isLoading /></MemoryRouter>);
    expect(screen.getByText("发现研究空白中…")).toBeInTheDocument();
    expect(screen.queryByText("尚未发现研究空白")).toBeNull();

    rerender(<MemoryRouter><GapPanel projectId={5} gaps={[]} error={new Error("发现失败")} /></MemoryRouter>);
