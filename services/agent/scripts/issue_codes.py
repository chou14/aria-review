...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="批量生成积分兑换码")
    parser.add_argument("--credits", type=int, required=True, help="每个兑换码的积分面值")
    ...
    try:
        async with factory() as s:
            ...
            try:
                ...
                await s.rollback()
                print("生成失败：兑换码与已有记录冲突，请重新执行。", file=sys.stderr)
                return 1
    finally:
