...
# ===========================================================================
# P1-4b: OpenAlex cursor 分页聚合（fetch_page 可注入，不触网）
# ===========================================================================

test_that(".oa_collect_cursor_pages 使用 cursor 聚合多页且每页 per-page=200", {
  calls <- list()
  mk <- function(from, to) lapply(from:to, function(i) list(id = paste0("W", i)))
  fetch_page <- function(cursor, per_page) {
    calls[[length(calls) + 1L]] <<- list(cursor = cursor, per_page = per_page)
    if (length(calls) == 1L) {
      list(results = mk(1L, 200L), next_cursor = "cursor-2")
    } else {
      list(results = mk(201L, 400L), next_cursor = "cursor-3")
    }
  }

  works <- .oa_collect_cursor_pages(250L, fetch_page, max_elapsed_s = 999)

  expect_equal(length(works), 250L)
  expect_equal(vapply(calls, `[[`, integer(1), "per_page"), c(200L, 200L))
  expect_equal(vapply(calls, `[[`, character(1), "cursor"), c("*", "cursor-2"))
  expect_false(.oa_partial(works))
})

test_that(".oa_collect_cursor_pages 遇到 429 返回已聚合部分结果", {
  calls <- 0L
  fetch_page <- function(cursor, per_page) {
    calls <<- calls + 1L
    if (calls == 1L) {
      return(list(results = lapply(1:200, function(i) list(id = paste0("W", i))),
                  next_cursor = "cursor-2"))
    }
    .oa_fetch_error(429L, "rate limit")
  }

  works <- .oa_collect_cursor_pages(250L, fetch_page, max_elapsed_s = 999)

  expect_equal(length(works), 200L)
  expect_true(.oa_partial(works))
  expect_match(.oa_partial_reason(works), "429")
})

test_that(".oa_collect_cursor_pages 首页 429 返回空的部分结果而非错误", {
  fetch_page <- function(cursor, per_page) .oa_fetch_error(429L, "rate limit")

  works <- .oa_collect_cursor_pages(100L, fetch_page, max_elapsed_s = 999)

  expect_equal(length(works), 0L)
  expect_true(.oa_partial(works))
  expect_match(.oa_partial_reason(works), "429")
})

test_that(".oa_collect_cursor_pages 总体超时返回已聚合部分结果", {
  calls <- 0L
  clock_values <- c(0, 0, 100)
  clock_i <- 0L
  clock <- function() {
    clock_i <<- clock_i + 1L
    clock_values[min(clock_i, length(clock_values))]
  }
  fetch_page <- function(cursor, per_page) {
    calls <<- calls + 1L
    list(results = lapply(1:200, function(i) list(id = paste0("W", i))),
         next_cursor = "cursor-2")
  }

  works <- .oa_collect_cursor_pages(250L, fetch_page, max_elapsed_s = 30,
                                    clock = clock)

  expect_equal(calls, 1L)
  expect_equal(length(works), 200L)
  expect_true(.oa_partial(works))
  expect_match(.oa_partial_reason(works), "超过 30 秒")
})

test_that(".oa_collect_cursor_pages limit > 500 显式错误", {
  err <- .oa_collect_cursor_pages(501L, function(...) stop("不应触发网络"))

  expect_true(.oa_is_error(err))
  expect_equal(err$status, 400L)
  expect_match(err$message, "500")
})
...
test_that("oa_search_candidates 透传部分结果元数据", {
  library(mockery)
  works <- list(list(id = "https://openalex.org/W123", title = "Partial Paper"))
  attr(works, "partial") <- TRUE
  attr(works, "partialReason") <- "OpenAlex 返回 429: rate limit"
  stub(oa_search_candidates, ".oa_search_works",
       function(query, n, since) works)

  result <- oa_search_candidates("test", n = 5L)

  expect_equal(length(result), 1L)
  expect_true(.oa_partial(result))
  expect_match(.oa_partial_reason(result), "429")
})

# ===========================================================================
# oa_search_candidates n 上限 500（超过显式报错, 不静默截断）
# ===========================================================================

test_that("oa_search_candidates n > 500 显式报错", {
  library(mockery)
  calls <- list()
  stub(oa_search_candidates, ".oa_search_works",
  ...
  expect_match(err, "OPENALEX_LIMIT_EXCEEDED")
})

test_that("oa_search_candidates n = 100 保持不变", {
  library(mockery)
  calls <- list()
  stub(oa_search_candidates, ".oa_search_works",
       function(query, n, since) { calls[[length(calls) + 1L]] <<- n; list() })
